...
- Cookie refresh bật `Secure`; đăng ký công khai được giữ nguyên.
...
4. Thêm biến môi trường Render (secret nhập trực tiếp trong dashboard):
   ...
   Không cần cấu hình SMTP. Không bật ping cron để giữ Render thức.

5. Workflow GitHub Actions hiện có build/lint frontend rồi upload lên Pages; sau khi push thay đổi trong `web-app/` lên `master`, xác nhận job Cloudflare Pages thành công. Workflow kiểm thử backend với PostgreSQL rồi gọi Render deploy hook khi có thay đổi trong `backend/`. Render hook chỉ khởi chạy deploy; kiểm tra dashboard Render để xác nhận deployment mới ở trạng thái `Live`.
...
- Nếu SMTP app password cũ từng được lưu trong cấu hình hoặc chia sẻ, thu hồi tại nhà cung cấp. Sau khi phiên bản mới chạy ổn định, xóa các biến SMTP/recovery cũ khỏi Render.
